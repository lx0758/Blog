...
export const addArticle = (
  ...
  requestBody(
    ...
    'POST',
    null,
    createFormData(title, categoryId, url, tags, content, weight, enableComment, status)
  )
// 更新文章
...
// 更新评论(审核通过)
export const updateCommentToVerify = (id: number) =>
...
export const queryCategoryOptions = () =>
  queryCategory(null, 1, 0x7fffffff, null, null).then((data) => {
...
export const addFile = (file: any) => {
  ...
  return requestBody('/api/file', 'POST', null, formData)
}
// 上传文件
export const updateFile = (id: number, file: any) => {
  const formData = new FormData()
  formData.append('file', file)
  return requestBody('/api/file/' + id, 'PUT', null, formData)
}
// 删除文件
export const deleteFile = (id: number) => request('/api/file/' + id, 'DELETE', null, null)
...
// 新增配置
export const addConfig = (key: string, value: string, description: string) =>
